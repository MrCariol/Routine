...
- Un solo dominio configurabile dall'utente: quello dell'**hub di autenticazione** (`RoutineAuth.getHubDomain`/`setHubDomain`, chi garantisce l'identità). Il **backend** (`api/sync.php`) vive sempre sullo stesso dominio che serve l'app e viene chiamato via path relativo (`js/sync.js`): niente CORS, richieste sempre same-origin.
